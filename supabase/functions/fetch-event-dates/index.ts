  ...
  try {
    ...
    for (const event of events) {
      ...
      try {
        ...
        // Simplified query approach - focus on HTML pages only (skip PDFs)
        ...
        while (!dateFound && queryIndex < Math.min(queries.length, 3)) {
          // Limit to 3 queries per event to save time
          if (Date.now() - startTime > MAX_PROCESSING_TIME) {
            break;
          }
          ...
          // METHOD 1: Try Google Search scraping
          try {
          ...
          // METHOD 2: Try Google Custom Search API for snippets (faster than scraping full pages)
          if (!dateFound) {
            try {
